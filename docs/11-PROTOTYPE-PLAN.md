...
- **Неделя 1 — ✅ завершена 2026-09-09.** `sim/DevAInt.Sim` + xUnit-тесты
  (92 теста, ~150 мс): `Hex` (аксиал, odd-r, линия), `Layer<T>`, `Rng`
  (xorshift64*), `Rules.Load` с валидацией восьми JSON-файлов, `World`,
  `Search.Bfs`, `MapGen.Generate` (центры, роли по farthest-point, заливка,
  коридоры по остовному дереву, линки, валидация с повтором сида, ASCII-дамп).
  По ходу пересмотрены числа генератора: радиус кластера 2–3, дистанция
  центров 7, дуэль 26×20, кластеров ≈ 2N + 3 — иначе кластеры сливались.

## Что режем без жалости (в 8 недель НЕ входит)

Сценарии, онлайн-сеть, динамическая дипломатия, партии на 3+ фракций
в UI (правила и генератор — N-ready с недели 1, проверяются авто-партиями),
сейвы/отмотка, звук, меню и онбординг,
...
- `Rules.Load` + все `*.json` с числами из `07-BALANCE.md`; валидация
  данных как тест.
- `MapGen` от сида: кластеры, роутеры-коридоры, линки, N стартов
  (farthest-point), имена; тест связности/достижимости на 2, 4 и 8
  стартов; ASCII-дамп карты в консоль для глаз.
...
- `tools/Balance.cs` (`just balance`): 200 партий бот-против-бота,
  таблица метрик из `07-BALANCE.md §9`; плюс дымовой прогон на 4 (2×2)
  и 8 фракций — доигрывается, команды побеждают вместе.
...
- **Хотсит на двоих:** экран создания партии (слоты: контроллер, команда,
  цвет), экран передачи хода, точка зрения рендера по активной фракции.
  Это же проверяет, что UI не привязан к «фракции 0».
...
3–8 фракций в UI, ИИ с заглядыванием, звук). Есть «нет» → чиним данные
