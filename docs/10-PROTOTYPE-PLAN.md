...
- **Неделя 3 — в работе.** Готово: память секторов (прирост излучение ×
  покрытие прямым вызовом, спад по тикам), битовая маска состояний робота
  с таблицей `EMISSION`, `just balance` (headless-прогон кривых) и
  калибровка `NOTICE_RATE = 2.5` (таблица в `07`). Впереди: пороги
  25/50 как события, шкала «Заметность в сети» и лог в HUD.
- Недели 4–8 — впереди.
