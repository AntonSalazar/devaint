...
Обычный класс, принадлежит `Main.gd`. **1 клетка грида = 1 клетка тайлмапа**
...
- Конверсии `cell_to_world`/`world_to_cell` реализованы и **бит-в-бит
  ...
  (`iso_test.gd`, 25×25 клеток + внутренние точки + round-trip).
...
- `tools/balance.gd` (`just balance`) — headless-прогон кривых заметности
