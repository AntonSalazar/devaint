...
4. **Композиция через `Main.cs`:** корневой скрипт владеет системами
...
```mermaid
flowchart TD
    Main["Main.cs — композиционный корень<br/>(явный порядок init/teardown)"]
...
- **`EventBus.reset()` обязателен и при выходе из игры** (teardown
  `Main.cs`, `NOTIFICATION_WM_CLOSE_REQUEST`): статическая шина,
...
- Обычный класс, принадлежит `Main.cs`.
...
Обычный класс, принадлежит `Main.cs`. **1 клетка грида = 1 клетка тайлмапа**
(решение недели 2: на квартале 40×40 укрупнение до 4×4 бессмысленно;
укрупним, если упрёмся на карте масштаба Малдро). Три слоя:

| Слой | Содержимое | Обновление | Статус |
|---|---|---|---|
| Статика | Сумма полей работающих вышек, линейный спад по наземному расстоянию, кламп 1.0 | Только по событию (`OnStaticChanged`) | ✅ |
| Динамика | Конусы дронов, фронт волны | Волна в симуляции — «фронт с координатой»; стена света — работа шейдера | впереди |
| Память секторов | По-секторная заметность 0–100, секторы 8×8 клеток (`07-SIGNATURE-MATH.md`) | Прирост — прямым вызовом `accumulate()` каждый кадр от источников; спад — по `OnMinutePassed` | ✅ |

- Грид выгружается в картинку (`get_static_image`, RF) → текстура шейдера;
  **`SignalLayer`** (Polygon2D + canvas-шейдер, `core/signal_layer/`) считает
  клетку той же формулой, что `Iso.world_to_cell`, из тех же констант
  (uniform'ы) и перезаливает текстуру только по `OnStaticChanged`.
  Симуляция и визуал физически не могут разъехаться — риск №1
  «читаемость невидимого» закрыт архитектурно и проверен чекпоинтом.
- **`Halo`** (`core/halo/`) — визуал излучения носителя: эллипс 2:1 на земле,
  радиус/частота пульса из излучения, плавное схождение к цели, фаза пульса
  копится в скрипте (не `TIME`), чтобы смена частоты не телепортировала кольцо.
  Визуальная формула живёт в `Halo`, носитель отдаёт только число —
  переиспользуем для агентов роя.
...
- `Iso` — статический класс (`core/iso/Iso.cs`): единая точка правды
...
- Конверсии `cell_to_world`/`world_to_cell` реализованы и **бит-в-бит
  сверены тестом с `TileMapLayer.map_to_local`/`local_to_map`**
  (`IsoTest.cs`, 25×25 клеток + внутренние точки + round-trip).
  Конвенция Godot Diamond Down: центр клетки (x, y) =
  ((x − y)·64 + 64, (x + y)·32 + 32); клетка (0,0) занимает прямоугольник
  от (0,0) до (128, 64). Обратная конверсия — округление к ближайшему центру.
- **Инвариант:** `World` и его TileMapLayer стоят в начале координат без
  трансформа — мировые координаты = локальные координаты слоя. Сдвиг World
  сломает конверсии; держим в (0,0) как закон.
...
- **Статические данные-таблицы вместо констант-россыпи** (пример:
  `Robot.DRAIN`, `Robot.EMISSION: Dictionary[State, float]`) — расширение
  без переделки, полнота таблиц проверяется тестом.
- **Состояние робота — битовая маска** (`Robot.State` как флаги, `_flags: int`):
  локомоция (IDLE/WALK) — взаимоисключающая пара в `LOCOMOTION_MASK`,
  модификаторы (будущие SCAN/CHARGE/…) — независимые биты. Таблицы
  аддитивны по поднятым битам: излучение = Σ `EMISSION[flag]`,
  расход = Σ `DRAIN[flag] × доля_минуты[flag]`.

## Тестовая инфраструктура

- `res://tests/`: раннер `test_runner.gd` (SceneTree, headless,
  `just test`), база `test_case.gd` (`check_*`, `before_each`,
  `wait_frames`), файлы `*_test.gd`; C#-наборы (`tests/*Test.cs`, наследники `CsTestCase`) гоняет мост `CsTestBridge.cs` через `csharp_test.gd`.
...
- `tools/Balance.cs` (`just balance`) — headless-прогон кривых заметности
  на настоящих `GameClock` + `SignalGrid`: первое боевое применение принципа
  «ядро без дерева сцены».
- Сцены тестируемы: `MainTest.cs` инстанцирует `main.tscn` в дерево
